...
/** Test if an item is successfully moved to the back 
* of the vector when the mouse is clicked on it.
* \param item The item that is clicked by a mouse
* \returns True if it is moved to the end, false otherwise
*/
bool CAquarium::ToEndTest(std::shared_ptr<CItem> item)
{
    if (mItems.back() == item)
    {
        return true;
...
/**
 * Move an item to the end of the item vector
 * \param item The item that is clicked by a mouse
 */
void CAquarium::ToEnd(std::shared_ptr<CItem> item)
{
...
void CAquarium::XmlItem(const std::shared_ptr<xmlnode::CXmlNode>& node)
{
    ...
    if (type == L"angelfish")
    {
        item = make_shared<CAngelFish>(this);
    }

    if (type == L"catfish")
    {
        item = make_shared<CCatFish>(this);
    }
